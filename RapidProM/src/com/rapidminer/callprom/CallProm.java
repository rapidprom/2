...
public class CallProm {
	...
	public CLIPluginContext instantiateProM_Context (File promLocation) {
		return instantiateCLI_Context(promLocation);
	}
	
	public Object[] runPlugin(PluginContext pc, String nameChild, String namePlugin, List<Object> parameters) {
		...
		Set<PluginParameterBinding> plugins = pc.getPluginManager().getPluginsAcceptingOrdered(child.getClass(),
				false,t);
		...
		if (results == null) {
			JOptionPane.showMessageDialog(null, "Plugin " + namePlugin + " did not return any result as an error occurred!");
		}
		return results;
	}
	
	public JComponent runVisualizationPlugin(PluginContext pc, String nameChild, List<Object> parameters) {
		...
		try {
			JComponent promVis = pc.tryToFindOrConstructFirstObject(JComponent.class, Connection.class, "", u);
			return promVis;
...
public JComponent runVisualizationPluginUsingName(PluginContext pc, String nameChild, String namePlugin, List<Object> parameters) {
		...
		try {
			JComponent promVis = pc.tryToFindOrConstructFirstNamedObject(JComponent.class, namePlugin, Connection.class, "u", u);
			return promVis;
		} catch (ConnectionCannotBeObtained e1) {
			e1.printStackTrace();
		}
	...
	private Class<?> getSubstitutedType (Class<?> type) {
		if (type.isAnnotationPresent(SubstitutionType.class)) {
			Class<?> declaredType = type.getAnnotation(SubstitutionType.class).substitutedType();
	...
	private CLIPluginContext instantiateCLI_Context(File promLocation) {
		try {
			LogService log = LogService.getGlobal();
			log.log("HERE callprom", LogService.NOTE);
			BootProM bswf = new BootProM(promLocation);
			String [] args = new String[0];
			Object obj;
			obj = bswf.bootForSWF(CLI_CallProM.class, CLIPluginContext.class, args);
			return (CLIPluginContext) obj;
		} catch (Throwable e) {
			e.printStackTrace();
			LogService log = LogService.getGlobal();
			log.log(e.getMessage(), LogService.ERROR);
		}
		return null;
	}

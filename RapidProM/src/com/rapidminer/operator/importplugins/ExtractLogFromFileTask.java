...
public class ExtractLogFromFileTask extends Operator {
	...
	private static final String[] FILE_TYPES = { "Naive", "Buffered", "Normal", "Lightweight", "Buffered_by_MAPDB" };
	...
	private InputPort proMcontext = getInputPorts().createPort("context (ProM Context)", ProMContextIOObject.class);
	...
	public ExtractLogFromFileTask(OperatorDescription description) {
		...
		// add additional rule that the file should be ok, see LoadFileOperator class
	}
...
//		logService.log("start do work read log task", LogService.NOTE);
		ProMContextIOObject context = proMcontext.getData(ProMContextIOObject.class);
		...
		try {
			if (fileType == Naive) {
				promLog = (XLog) (tp.runPlugin(pluginContext, "000", "Open XES Log File (Naive)", parameters))[0];
				//Object[] results = tp.runPlugin(pluginContext, "000", "Open XES Log File (Naive)", parameters);
			}
			else if (fileType == Buffered) {
				promLog = (XLog) (tp.runPlugin(pluginContext, "000", "Open XES Log File (Buffered)", parameters))[0];
			}
			else if (fileType == Normal) {
				promLog = (XLog) (tp.runPlugin(pluginContext, "000", "Open XES Log File", parameters))[0];
			}
			else if (fileType == Lightweight) {
				promLog = (XLog) (tp.runPlugin(pluginContext, "000", "Open XES Log File (Lightweight & Sequential IDs)", parameters))[0];
			}
			else if (fileType == Buffered_by_MAPDB) {
				promLog = (XLog) (tp.runPlugin(pluginContext, "000", "Open XES Log File (Disk-buffered by MapDB)", parameters))[0];
			}
		} catch (Throwable e) {
			e.printStackTrace();
		...
		if (valPar1.equals(PROM_VIS)) {
			...
			xLogIOObject.setVisualizationType(XLogIOObject.VisualizationType.ProM_Log_Visualizer);
		}
		else if(valPar1.equals(TABLE_VIS)) {
			xLogIOObject.setVisualizationType(XLogIOObject.VisualizationType.Table);
		}
		else{
			xLogIOObject.setVisualizationType(XLogIOObject.VisualizationType.XDottedChart);
		}
...
//		logService.log("end do work first prom task", LogService.NOTE);
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterTypeCategory filetypes = new ParameterTypeCategory(PARAMETER_FILE_TYPE, "select the format of the log file to be saved", FILE_TYPES, Naive);
		Object[] par1categories = new Object[] {TABLE_VIS, PROM_VIS, XDOT_VIS};
		ParameterCategory parameter1 = new ParameterCategory(par1categories, TABLE_VIS,String.class, "Visualize Log", "Visualize Log");

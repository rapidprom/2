...
public class CPNStreamPublisherTask extends Operator{
	...
	private InputPort inputCPNModel = getInputPorts().createPort("model (ProM CPN model)", CPNModelIOObject.class);
	
	private OutputPort outputPublisher = getOutputPorts().createPort("publisher (ProM)");
	private OutputPort outputStream = getOutputPorts().createPort("Event Stream (ProM)");
	...
	public CPNStreamPublisherTask(OperatorDescription description) {
		...
		getTransformer().addRule( new GenerateNewMDRule(outputPublisher, XSPublisherIOObject.class));
		getTransformer().addRule( new GenerateNewMDRule(outputStream, MarkingIOObject.class));
...
public void doWork() throws OperatorException {
		...
		Object[] result = CPNModelToXSEventStreamAuthorPlugin.cpnToXSEventStreamPlugin(pluginContext, inputCPNModel.getData(CPNModelIOObject.class).getData(), parameters);
		
		outputPublisher.deliver(new XSPublisherIOObject((XSPublisher) result[0]));
		outputStream.deliver(new XSEventStreamIOObject((XSEventStream) result[1]));		

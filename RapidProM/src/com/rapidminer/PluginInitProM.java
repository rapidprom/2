...
//		// register prom location
		ParameterService.registerParameter(
			    new ParameterTypeDirectory("prom_folder",
...
//		loadRequiredClasses();
	...
	public static void initGui(MainFrame mainframe) {
		String parameterValue = ParameterService.getParameterValue("prom_folder");
		...
		if (!parameterValue.equals("")) {
			...
			// show dialog
			int showConfirmDialog = JOptionPane.showConfirmDialog(mainframe, "We did not detect RapidProM packages, have you installed them before?","Warning",JOptionPane.YES_NO_OPTION);
			if (showConfirmDialog == JOptionPane.YES_OPTION) {
				final JFileChooser fc = new JFileChooser();
				fc.setMultiSelectionEnabled(false);
				int showDialog = fc.showDialog(mainframe,"Please select the location of the ProM.ini file");
				...
				try {
				    ...
				    if (folderChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) { 
				    	InstallProM ip = new InstallProM(folderChooser, mainframe);
				    	ip.start();
				    }
				    else {
